...
/**
 * Returns true whenever the primary font cannot positively prove it covers the script glyph.
 * Unknown coverage is treated as needing fallback so imported/AI-generated text does not
 * silently render tofu while we wait for a definite CJK/Arabic fallback face.
 */
export function textNeedsFallbackScript(node: SceneNode, script: FontFallbackScript): boolean {
  ...
  for (let index = 0; index < node.text.length; index++) {
    ...
    if (fontGlyphCoverageSync(family, style, char) !== 'has') return true
  }
...
export function textNeededFallbackScripts(node: SceneNode): FontFallbackScript[] {
  ...
  for (let index = 0; index < node.text.length; index++) {
    ...
    const { family, style } = styleForCharacter(node, index)
    if (fontGlyphCoverageSync(family, style, char) === 'has') continue
